...
def plot_exp1(df):
    ...
    sns.barplot(
        x='desired coverage (1-α)', 
        y='desired coverage (1-α)', 
        data=df, 
        alpha=0.3, 
        ax=axs[0], 
        edgecolor='k', 
        ci=None,
        color='white'  # or remove 'fill=False'
    )
    # Left barplot -- empirical coverages
    bplot = sns.barplot(x='desired coverage (1-α)', y='coverage', hue='predictor', data=df, ax=axs[0], alpha=0.5, errorbar='sd', linewidth=0.01)
